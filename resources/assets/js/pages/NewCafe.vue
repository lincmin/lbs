      ...
      <div class="grid-container">
        <div class="grid-x grid-padding-x">
          <div class="large-12 medium-12 small-12 cell">
            ...
            <span
              class="validation"
              v-show="!validations.name.is_valid"
            >{{ validations.name.text }}</span>
          </div>
          <div class="large-12 medium-12 small-12 cell">
            <label>网址
              <input
          ...
          <div class="large-6 medium-6 small-12 cell">
            <label>详细地址
              <input
                type="text"
                placeholder="详细地址"
                v-model="locations[key].address"
              >
            </label>
            <span
              class="validation"
              v-show="!validations.locations[key].address.is_valid"
            >{{ validations.locations[key].address.text }}</span>
          </div>
          <div class="large-6 medium-6 small-12 cell">
            <label>城市
              <input
                type="text"
                placeholder="城市"
                v-model="locations[key].city"
              >
            </label>
            <span
              class="validation"
              v-show="!validations.locations[key].city.is_valid"
            >{{ validations.locations[key].city.text }}</span>
          </div>
          <div class="large-6 medium-6 small-12 cell">
            <label>省份
              <input
                type="text"
                placeholder="省份"
                v-model="locations[key].state"
              >
            </label>
            <span
              class="validation"
              v-show="!validations.locations[key].state.is_valid"
            >{{ validations.locations[key].state.text }}</span>
          </div>
          <div class="large-6 medium-6 small-12 cell">
            <label>邮编
              <input
                type="text"
                placeholder="邮编"
                v-model="locations[key].zip"
              >
            </label>
            <span
              class="validation"
              v-show="!validations.locations[key].zip.is_valid"
            >{{ validations.locations[key].zip.text }}</span>
          </div>
          <div class="large-12 medium-12 small-12 cell">
            <label>支持的冲泡方法</label>
            <span
...
export default {
  data() {
    return {
      name: "",
      locations: [], //存放所有新增的位置字段数据
      website: "", //咖啡店的网址
      description: "", //简介信息
      roaster: false, //是否有烘培师
      validations: {
        name: {
          is_valid: true,
          text: ""
        },
        locations: [], //包含每个位置字段的验证规则，确保添加的每个位置字段数据都是有效的
        oneLocation: {
          //用于确保咖啡店至少包含一个位置信息
          is_valid: true,
          text: ""
      ...
      for (var index in this.locations) {
        ...
        if (this.locations[index].city.trim() === "") {
          ...
          this.validations.locations[index].city.text =
            "Please enter a city for the new cafe!";
        } else {
        ...
        if (this.locations[index].state.trim() === "") {
          ...
          this.validations.locations[index].state.text =
            "Please enter a state for the new cafe!";
        } else {
        ...
        // 确保邮编字段不为空
        if (
          this.locations[index].zip.trim() === "" ||
          !this.locations[index].zip.match(/(^\d{6}$)/)
        ) {
          validNewCafeForm = false;
